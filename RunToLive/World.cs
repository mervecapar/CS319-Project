    ...
    public class World
    {
        ...
        public void changeStatusAfterTravelForPlayer()
        {
            ...
            if (mapManager.getMap().allCountries[cci].numberOfInfected > 0 || mapManager.getMap().allCountries[cci].numberOfSicked > 0)
                if (giveMePossibility(giveMePlayerInfectedProb())) //%sick people number in the country 
                {
        ...
        public int[] returnCurrentStatus()
        {
            ...
            foreach(Country c in  mapManager.gameMap.allCountries)
            {
                ...
                healthy = c.numberOfPeople - c.numberOfInfected - c.numberOfSicked - c.numberOfDied;
        ...
        public int giveMePlayerInfectedProb()
        {
            int totPeople = mapManager.gameMap.allCountries[mapManager.gameMap.player.currentCountryId].numberOfPeople;
            int sicPeople = mapManager.gameMap.allCountries[mapManager.gameMap.player.currentCountryId].numberOfSicked;
            int infPeople = mapManager.gameMap.allCountries[mapManager.gameMap.player.currentCountryId].numberOfInfected;
